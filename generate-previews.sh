...
extract_metadata() {
    ...
    if [[ $script_name =~ ^blackroad ]]; then
        ...
        category="monitoring"
    elif [[ $script_name =~ (ai|agent|neural|brain) ]]; then
        category="ai"
    elif [[ $script_name =~ (crypto|blockchain|bitcoin) ]]; then
        category="crypto"
    elif [[ $script_name =~ (api|network|cloud|docker|database) ]]; then
        category="infrastructure"
    elif [[ $script_name =~ (graph|chart|visual|3d|heatmap) ]]; then
        category="visualization"
    elif [[ $script_name =~ (security|alert|anomaly) ]]; then
        category="security"
